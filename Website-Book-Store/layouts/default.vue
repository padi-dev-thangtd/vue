        ...
        <div class="container d-flex">
          ...
          <div class="header-right d-flex align-items-center ml-3">
            <div>
              <div
                v-if="!loggedIn"
                class="header-right__user d-flex mr-3"
                @click="handleShowFormLogin"
              >
                <img
              ...
              <a-tooltip placement="bottomLeft" v-else>
                <template slot="title">
                  ...
                  <div
                    v-if="dataToken ? !(dataToken.permission === 'admin') : true"
                    class="d-flex mb-2 align-items-center"
                  ...
                  v-if="dataToken ? !(dataToken.permission === 'admin') : true"
                    class="d-flex mb-2 align-items-center"
                    @click="$router.push('/user/voucher/')"
                  >
                  ...
                  <div   v-if="dataToken ? !(dataToken.permission === 'admin') : true">
                    ...
                    <div
                      class="d-flex align-items-center"
                      @click="$router.push('/admin/orderStatus/')"
                    >
            ...
            <div
                v-if="dataToken ? !(dataToken.permission === 'admin') : true"
              class="header-right__cart d-flex"
        ...
        <div class="form-register__content">
          ...
          <div class="form-register__content--email">
            ...
            <input
              ...
              type="text"
              class="form-register__content--email-input form-control"
          ...
          <div
            v-if="isShowRegister"
            class="form-register__content--email pt-2 pb-3"
          >
          ...
          <div v-if="isShowLogin" class="text-center mt-4">
            <p class="font-12px">
              Bằng việc đăng nhập, bạn đã đồng ý với Fahasa.com về
            </p>
            <p class="font-12px">
              <a href="#" class="txt-origin">Điều khoản dịch vụ</a> &
              <a href="#" class="txt-origin">Chính sách bảo mật</a>
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  async created() {
    ...
    if (dataLogin) {
      ...
      let dataCart = await this.$store.dispatch(
        "users/getCart",
        dataLogin.token
      );
    ...
    handleSearch() {
      if (this.dataForm.search === "") {
        ...
        const dataSearch = this.listProducts.filter(it => {
          return (
            it.name.includes(this.dataForm.search) ||
            it.auth.includes(this.dataForm.search)
          );
        });
