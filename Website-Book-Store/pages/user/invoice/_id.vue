<template>
  <div class="p-3">
    <div class="invoice-info mb-4">
      <p
        class="d-flex align-items-center font-16px txt-black-3 cursor-pointer"
      ...
      <div class="font-16px">
        <span
          >ID ĐƠN HÀNG: {{ invoiceInfo ? invoiceInfo._id : "" }}&nbsp;
          &nbsp;|</span
        >
        <span class="txt-red ml-2">{{
          invoiceInfo
            ? invoiceInfo.invoiceDetail.length == 0
              ? "ĐANG XỬ LÝ"
              : "ĐÃ GIAO"
            : ""
        }}</span>
    ...
    <div class="mb-4">
      <a-steps :current="handleCurrent(invoiceInfo)" status="error" class="p-2">
        <a-step>
          ...
          <p slot="title" :class="dataOrder ? 'txt-green' : ''">
            {{ dataOrder ? "Thành công" : "Đang chờ" }}
          </p>
        ...
        <a-step>
          ...
          <p slot="title" :class="dataDilivery ? 'txt-green' : ''">
            {{ dataDilivery ? "Thành công" : "Đang chờ" }}
          </p>
        ...
        <a-step>
          ...
          <p slot="title" :class="dataInvoice ? 'txt-green' : ''">
            {{
              dataInvoice ? "Thành công" : checkCancel ? "Đã hủy" : "Đang chờ"
            }}
          </p>
    ...
    <div class="mt-4 p-2 mb-3">
      <p class="font-20px font-weight-bold mb-3">Danh sách sản phẩm đã mua</p>
      <div v-if="listOrder.length > 0">
        <base-carousel :slideToShow="4" :listStory="listOrder || []" />
      </div>
      <div v-else>
        <a-empty />
      </div>
...
export default {
  ...
  async mounted() {
    ...
    this.invoiceInfo = await this.$store.dispatch(
      "users/getInvoiceId",
      dataInvoiceId
    );
    this.listOrder = this.invoiceInfo.order;
