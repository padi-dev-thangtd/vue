      ...
      <div class="card-container">
          ...
          <a-tab-pane key="1" tab="Tất cả">
            ...
            <section class="bg-white cart-user">
              <div
                ...
                v-for="invoice in dataInvoiceAll"
                :key="invoice._id"
              >
        ...
        >
          <p>
            Bạn chắc chắn muốn
            {{
              ["confirm", "transport", "complete"].includes(activeButton)
                ? "xác nhận"
                : "hủy"
            }}
            đơn hàng này không ?
...
export default {
  ...
  methods: {
    ...
    async handleOk() {
      ...
      this.dataTransport = this.dataInvoiceAll.filter(it => {
        return (
          it.invoiceDetail.length === 1 && it.invoiceDetail[0].status === 1
        );
      });
      this.dataComplete = this.dataInvoiceAll.filter(it => {
        return (
          it.invoiceDetail.length === 2 && it.invoiceDetail[1].status === 2
        );
      });
      this.dataCancel = this.dataInvoiceAll.filter(it => {
        return it.invoiceDetail.some(item => item.status === 4);
      ...
      this.dataComplete1 = this.dataInvoiceAll.filter(it => {
        return (
          it.invoiceDetail.length === 3 && it.invoiceDetail[2].status === 3
        );
      });
