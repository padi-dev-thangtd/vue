<template>
  <div>
    <a-breadcrumb style="margin: 16px 0">
      ...
      <a-breadcrumb-item class="font-16px text-underline cursor-pointer">Edit user</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <div class="font-24px text-center mb-3">Edit user</div>
      <a-form-model
        ref="ruleForm"
        :model="ruleForm"
        :rules="rules"
      ...
      >
        ...
        <a-form-model-item ref="permission" label="Thể loại" prop="permission">
...
export default {
  ...
  async created() {
    let dataUserInfo = await this.$store.dispatch('users/getUserInfo', this.$route.params.id);
    Object.assign(dataUserInfo, {checkPass: dataUserInfo.password, phone: `0${dataUserInfo.phone}`})
    this.ruleForm = {...dataUserInfo}
  },
  ...
  methods: {
    ...
    onSubmit(ruleForm) {
      this.$refs[ruleForm].validate(async valid => {
        if (valid) {
          await this.updateUser(this.ruleForm);
