<template>
  <div>
    <a-breadcrumb style="margin: 16px 0">
      <a-breadcrumb-item class="font-16px text-underline cursor-pointer">Vouchers</a-breadcrumb-item>
    </a-breadcrumb>
    <div class="text-right mb-4 mr-3">
      <b-button variant="outline-success" @click="$router.push('/admin/vouchers/add/')">Add voucher</b-button>
    </div>
    <a-table
      :columns="columns"
    ...
    >
      ...
      <span slot="nameVoucher" class="font-14px font-weight-bold" slot-scope="text" >{{ text }}</span>
      <span slot="baseDate" class="font-14px" slot-scope="text">{{ text }}</span>
      <span slot="apply" class="font-14px" slot-scope="text">{{ text | numberWithCommas }}đ</span>
      ...
      <div slot="action" slot-scope="action" class="txt-right pr-3">
        <nuxt-link :to='`/admin/vouchers/${action._id}/edit`'><u> Edit</u></nuxt-link>
