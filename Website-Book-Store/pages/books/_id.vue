    ...
    <div class="mt-4">
      <div class="product-view bg-white">
        <div class="product-view__image">
          <div class="product-view__image-img">
            <img :src="productInfo ? productInfo.image : ''" alt="" width="100%" loading="lazy">
          </div>
          <div class="product-view__image-btn mt-3">
            <b-button pill variant="outline-primary" class="add-cart mr-3" @click="handleAddCart">
              Thêm vào giỏ hàng
            </b-button>
            <b-button pill variant="info" class="buy" @click="handleAddCart">Mua ngay</b-button>
            <a-modal
              title="Thông Báo"
        ...
        <div class="product-view__detail">
          <div class="product-view__detail-name mb-3">Cân Bằng Cảm Xúc, Cả Lúc Bão Giông</div>
          <div class="product-view__detail-page mb-2">
            <p>Xuất xứ: <span class="font-weight-bold">Việt Nam</span></p>
            <p>Tác giả: <span class="font-weight-bold">{{ productInfo ? productInfo.auth : '' }}</span></p>
          </div>
          <div class="product-view__detail-page mb-2">
            <p>Hình thức: <span class="font-weight-bold">Bìa Mềm</span></p>
          </div>
          <div class="product-view__detail-rate mb-3">
            <b-form-rating variant="warning" inline :value="productInfo ? productInfo.rate : ''" disabled></b-form-rating>
            <span class="evaluate">( 24 đánh giá )</span>
          </div>
          <div class="product-view__detail-price mb-3">
            <span class="real-price">{{ (productInfo ? productInfo.price : '' ) | numberWithCommas }}đ</span>
            <span class="reduced-price">{{ productInfo ? Math.round(productInfo.price / ((100-parseInt(productInfo.sale))/100)) : '' }}</span>
          ...
          <div class="mb-1 cursor-pointer">
            <b-icon icon="star-fill" animation="fade" font-scale="1" class="txt-purple"></b-icon>
            <span class="txt-blue font-weight-bold ml-2">Nhập mã "QRFHS2": Giảm ngay 10K, đơn hàng từ 50K, thanh toán qua VNPAY</span>
          </div>
          <div class="mb-1 cursor-pointer">
            <b-icon icon="star-fill" animation="fade" font-scale="1" class="txt-purple"></b-icon>
            <span class="txt-blue font-weight-bold ml-2">Hoàn 30%, tối đa 50K, thanh toán qua ví Moca</span>
          </div>
          <div class="mb-1 cursor-pointer">
            <b-icon icon="star-fill" animation="fade" font-scale="1" class="txt-purple"></b-icon>
            <span class="txt-blue font-weight-bold ml-2">Nhập mã "AIRPAY021", Giảm ngay 10%, đơn hàng từ 0đ, thanh toán qua ví AirPay</span>
          </div>
          <div class="mb-1 cursor-pointer">
            <b-icon icon="star-fill" animation="fade" font-scale="1" class="txt-purple"></b-icon>
            <span class="txt-blue font-weight-bold ml-2">Giảm ngay 20K, đơn hàng từ 40K, thanh toán qua Ví ZaloPay</span>
          </div>
        </div>
      </div>
      <div class="product-tabs mt-4">
        <div>
          <b-card no-body>
            <b-tabs pills card >
              <b-tab title="Thông tin sản phẩm" active><b-card-text>
                ...
                <div class="product-tabs__rows">
                  ...
                  <p class="txt-origin">Top 100 sản phẩm bán chạy của tháng</p>
                </div>
                <hr>
                <div class="product-tabs__content">
                  <p class="product-tabs__content-title mb-2">MÔ TẢ SẢN PHẨM</p>
                  <p class="product-tabs__content-text">{{ productInfo ? productInfo.content : '' }}</p>
                </div>
              </b-card-text></b-tab>
                ...
                <b-card-text>
                  <div class="product-tabs__evaluate">
                    <div class="product-tabs__evaluate-point">
                      ...
                      <b-form-rating variant="warning" inline value="4.8" disabled></b-form-rating>
                      <p class="ml-1 pt-2">97 review</p>
                    </div>
                    <div class="product-tabs__evaluate-progress">
                      <div class="evaluate-rate">
                        <p>5 sao</p>
                        <b-progress height="6px" value="70" variant="warning" key="1"></b-progress>
                        <p class="pl-3">70%</p>
                      </div>
                      <div class="evaluate-rate">
                        <p>4 sao</p>
                        <b-progress height="6px" value="50" variant="warning" key="1"></b-progress>
                        <p class="pl-3">50%</p>
                      </div>
                      <div class="evaluate-rate">
                        <p>3 sao</p>
                        <b-progress height="6px" value="30" variant="warning" key="1"></b-progress>
                        <p class="pl-3">30%</p>
                      </div>
                      <div class="evaluate-rate">
                        <p>2 sao</p>
                        <b-progress height="6px" value="0" variant="warning" key="1"></b-progress>
                        <p class="pl-3">0%</p>
                      </div>
                      <div class="evaluate-rate">
                        <p>1 sao</p>
...
export default {
  ...
  async created() {
    this.productInfo = await this.$store.dispatch('products/getProductId', this.$route.params.id);
  },
  ...
  methods: {
    ...
    async handleAddCart() {
      ...
      if (dataLogin) {
        ...
        let checkCartExist = _.filter(getCart, { productId: this.$route.params.id });
