    ...
    <div class="bg-white mt-5 p-4">
      ...
      <div>
        ...
        >
          ...
          <a-form-model-item ref="address" label="Địa chỉ nhận hàng" prop="address">
        ...
        <div v-for="cart in dataCart" :key="cart._id" class="d-flex align-items-center cart-container__content">
          ...
          <div class="cart-content w-100" @click="$router.push(`/books/${cart.productId}`)">
            ...
            <div class="cart-content__info">
              ...
              <p class="font-16px font-weight-bold">{{ cart.price | numberWithCommas }}</p>
            </div>
    ...
    <div class="bg-white mt-4 p-4">
      ...
      <div>
        ...
        <div class="payment flex-end">
          ...
          <p class="font-22px txt-origin">{{ totalMoney | numberWithCommas }} đ</p>
        </div>
        <div class="d-flex flex-space-between mt-3">
          <b-button pill variant="outline-secondary" @click="$route.hash === '#info' ? $router.go(-1) : $router.push('/checkout/cart/')">Quay lại </b-button>
          <b-button v-if="$route.hash !== '#info'" variant="danger" @click="hanhdlePayment">XÁC NHẬN THANH TOÁN</b-button>
        </div>
      </div>
    </div>
    <a-modal
...
export default {
  ...
  async mounted() {
    ...
    this.dataUser = await this.$store.dispatch('users/getUserInfo', dataLogin.token);
    if (this.dataUser) {
      this.form = {
    ...
    this.dataCart = await this.$store.dispatch('users/getCart', dataLogin.token);
    ...
    if (this.useVoucher) {
      if (this.useVoucher.nameVoucher.toLowerCase().includes('coupon')) {
        this.totalCoupon = _.replace(_.split(this.useVoucher.nameVoucher, ' ')[1], 'k', '000');
    ...
    this.totalMoney = this.totalMoneyBook + this.totalFreeship - this.totalCoupon;
  ...
  methods: {
    ...
    handleUseVoucher(voucher) {
      ...
      if (voucher.nameVoucher.toLowerCase().includes('coupon')) {
        this.totalCoupon = _.replace(_.split(voucher.nameVoucher, ' ')[1], 'k', '000');
        this.totalFreeship = 30000;
      } else {
        this.totalFreeship = 0;
        this.totalCoupon = 0;
